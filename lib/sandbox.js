...
/**
 * 读当前旋钮快照（离开只读时按此还原）。
 * @returns `{presetName, sandbox, approval}`，缺失的服务对应字段为 undefined。
 */
function snapshot({ sandboxPolicy, approval, permissionPresets }, session) {
  ...
  return { presetName, sandbox: mode, approval: policy }
}
...
export function createHardGate({ permissionPresets, sandboxPolicy, approval }) {
  ...
  return {
    ...
    leave(agent, descriptor) {
      ...
      const canRestorePreset = permissionPresets !== undefined
        && typeof saved.presetName === 'string'
        && saved.presetName !== CUSTOM_PRESET
        && (() => {
          try {
            return permissionPresets.names.includes(saved.presetName)
          } catch {
            return false
          }
        })()
      if (canRestorePreset) {
        permissionPresets.set(session, saved.presetName)
        return 'preset'
      }
