 ...
 * @returns 命令结果。
 */
function togglePlanMode(ctx, states, agent) {
  const planMode = planModeOf(ctx, agent)
  if (planMode === undefined) {
    return failure('当前会话的 preset 没有挂载 plan mode（@deepseek-ai/dsh-plan-mode），无法切换。')
  }
  const sessionId = agent.session.id
  const current = states.get(sessionId).mode
  const target = nextToggleMode(current)
  const outcome = planMode.set(agent, target === MODE.planning)
  states.set(sessionId, target)
  return target === MODE.planning
    ? success(`plan 模式已开启（${outcome}）。先聊清楚需求，然后按 Alt+2 复核并执行；再按 Alt+1 退出。`)
    : success(`plan 模式已关闭（${outcome}），流水线状态已清空。`)
}
 ...
 */
async function runPlanReview(ctx, states, agent, signal) {
  const sessionId = agent.session.id
  ...
  states.set(sessionId, MODE.reviewing)
  try {
    ...
    return success(
      `复核完成：${review.findings.length} 条发现（${fixed} 条已修订）→ 已归档 ${archived.path}（${archived.mode}）→ 执行轮已启动。`,
    )
  } catch (error) {
    // 失败一律回到 planning：保留需求讨论上下文，让用户能直接重试或手改。
    if (states.get(sessionId).mode === MODE.reviewing) states.set(sessionId, MODE.planning)
...
export function apply(ctx) {
  ...
  ctx.effect(() => ctx.commands.register({
    ...
    handler: ({ agent }) => togglePlanMode(ctx, states, agent),
  }), `${name}: /plan-toggle`)

  ctx.effect(() => ctx.commands.register({
    name: 'plan-review',
    description: 'Plan review pipeline (Alt+2): subagent review, archive, auto-execute, self-review.',
    handler: ({ agent, signal }) => runPlanReview(ctx, states, agent, signal),
  }), `${name}: /plan-review`)
