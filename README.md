...
- **插件重载 / `dsh web` 重启会丢失插件的内存状态**，而沙箱覆盖写在会话日志里、跨重启保留。若重启前停在只读态，
  插件下次接触到该会话时会“采纳”这个既存状态；退出时因为**进入前的状态已不可知**，会按
  「本次进程里上次见到的正常预设 → 部署默认预设（`permission.defaultPreset`）→ 部署默认旋钮」逐级还原，
  **不会把会话卡在只读里**（线上曾出现过把只读快照当成还原目标、导致永久只读的 bug，已修并有回归用例）。
  残留局限：重启后如果你之前手动选过非默认预设（例如 `workspace-write`），退出会回到**部署默认预设**而非那个手选值；
  此时在 UI 的权限预设选择器里再选一次即可。
